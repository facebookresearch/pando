...
from xplat.frl.bci.doc import fill_in_docstring
...
_argument_doc = fill_in_docstring(
    {
      ...
      """,
        "base_cls": """base_cls: df.Message = df.Message
...
class FunctionToNode:
    ArgumentMessage: Optional[df.Message]
    ReturnMessage: Optional[df.Message]
    FunctionNode: df.Node

    def __iter__(self):
        return iter(astuple(self))


class FunctionConfig(df.Config):
    kwargs: Dict[str, Any] = field(default_factory=dict)


class _BlankMesage(df.Message):
    pass
...
def parse_argument_annotations(
    ...
    be passed to the final df.Node through a df.Message and which should be
...
def parse_return_annotation(
    ...
    return type directly into the final return df.Message fields for
...
def extract_message_fields(message: df.Message, fields: List[str]) -> Dict[str, Any]:
...
class _BlankMessage(df.Message):
...
def function_to_node(
    f: Callable[[Any], Any],
    base_cls: df.Message = df.Message,
    keyword_only_in_message: bool = False,
    default_keyword_in_message: bool = False,
    deconstruct_dataclass_return_type: bool = True,
    single_return_param_name: str = "sample",
    ...
    class ArgumentMessage(df.Message):
        sample: int

    class ReturnMessage(df.Message):
        sample: int

    class FunctionNode(df.Node):
        INPUT = df.Topic(ArgumentMessage)
        OUTPUT = df.Topic(ReturnMessage)
        ...
        @df.subscriber(INPUT)
        @df.publisher(OUTPUT)
        async def run(self, message: ArgumentMessage) -> df.AsyncPublisher:
    ...
    if argument_types is not None:
        argument_topic = df.Topic(ArgumentMessage)
        topics[argument_topic_name] = argument_topic

    if return_types is not None:
        return_topic = df.Topic(ReturnMessage)
    ...
    def call(message, kwargs):
        ...
        # `df.TimestampedMessage`, then the field `timestamp` is passed through
