...
import pytest
from bci.nodes.function_to_node import (
    FunctionConfig,
    function_to_node,
...
# correctly.
class SampledIndexMessage(df.Message):
    sample_index: int
...
def test_function_returns_same_after_conversion() -> None:
    ...
    tests that a base class that is not `df.Message` has its parameters passed
    through."""
    ...
    test_harness = df.NodeTestHarness(FunctionNode)
    ...
    with test_harness.get_node(config=FunctionConfig(kwargs={"b": b})) as node:
        ...
        node_to_value_msg = df.run_async(node.run, args=[msg])[0][1]
...
class SourceConsumer(df.Node):
    INPUT = df.Topic(SourceReturnMessage)
...
class SinkGenerator(df.Node):
    OUTPUT = df.Topic(SinkArgumentMessage)
...
class InputOutputGenerator(df.Node):
    OUTPUT = df.Topic(InputOutputArgumentMessage)
...
class InputOutputConsumer(df.Node):
    INPUT = df.Topic(InputOutputReturnMessage)
...
class InputOutputGraph(df.Graph):
