...
class ChannelInfo(object):
    ...
    modality's channel information. The mental model to have
    is a 2D array, where each row describes all the
...
class DOTChannelInfo(ChannelInfo):
    ...
    def __init__(
        ...
        self.check_and_assign("canister_idx", canister_idx, "int")
...
class HemoglobinChannelInfo(ChannelInfo):
    # Essentially the same as DOTChannelInfo, but now
    # labels whether a channel corresponds to HbO or not
    ...
    detector_y: np.ndarray  # float
    detector_z: np.ndarray  # float
    is_HbO: np.ndarray  # bool -- True if HbO, False if HbR
    canister_idx: np.ndarray  # int
    ...
    def __init__(
        ...
        if not np.all(args_len == args_len[0]):
            raise ValueError("The inputs need to all have the same length!")
        super().__init__("Flattened Hemoglobin data", args_len[0])
        ...
        self.check_and_assign("detector_y", detector_y, "float")
        self.check_and_assign("detector_z", detector_z, "float")
        self.check_and_assign("is_HbO", is_HbO, "bool")
        self.check_and_assign("canister_idx", canister_idx, "int")
        self.device_name = device_name
